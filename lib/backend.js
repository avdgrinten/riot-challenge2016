...
let Backend = class {
	constructor(opts) {
		...
		this._requireUserInLobbyMiddleware = (req, res, next) => {
			if(!req.lobby.containsUser(req.user)) {
	...
	_setupLobbyRouter() {
		...
		router.get('/site',
			this._requireUserMiddleware,
			this._requireUserInLobbyMiddleware,
			(req, res) => {
				res.json({
					user: {
						displayName: req.user.summoner.displayName,
					...
					ownIndex: req.lobby.indexOfUser(req.user)
				});
			}
		);
		...
		router.post('/ready',
			this._requireUserMiddleware,
			(req, res) => {
				// TODO: handle errors from startGame()
				req.lobby.startGame();
		...
		router.post('/join',
			this._requireUserMiddleware,
			(req, res) => {
				let success = req.lobby.joinUser(req.user);
				if(!success)
					return res.json({
						error: 'join-failed'
					});
		...
		router.post('/lock-answer',
			...
			this._jsonMiddleware,
			(req, res) => {
				req.lobby.lockAnswer(req.user, req.body.answer);
