...
let BaseCrawler = class {
	...
	updateMasteries(platform_id, summoner_id) {
...
let BackgroundCrawler = class extends BaseCrawler {
	...
	run() {
		this._crawlSome()
		.catch(error => {
			console.error("Error while crawling for " + this._platformId);
			console.error(error);
		})
		.then(() => {
			this.run();
	...
	_ensureSomeSummoners() {
		...
		.then(count => {
			...
			.catch(error => {
				console.error("Error while gathering data for " + this._platformId + "."
						+ " Retrying.");
				console.error(error);
			})
			.then(() => this._ensureSomeSummoners());
	...
	_crawlSome() {
		...
		.then(array => {
			if(array.length > 0) {
//				console.log("Importing mastery data of " + array.length
//						+ " summoners on " + this._platformId);

				return Promise.all(array.map(summoner => {
					return this.updateMasteries(this._platformId, summoner.summonerId);
				}));
			}else{
				return this._summoners.count({
					platformId: this._platformId,
...
//				console.log("Expansion yields " + ids.length
//						+ " summoners on " + this._platformId);
...
//			console.log("Purging random summoners from " + this._platformId);
			return this._summoners.aggregate([
				{ $match: { platformId: this._platformId } },
				{ $sample: { size: 100 } }
			]).toArray()
