...
let BackgroundCrawler = class extends BaseCrawler {
	...
	initialize() {
		...
		.then(() => console.log(this._platformId, "OK"));
	}
	...
	_ensureEnoughApplicable() {
		return Promise.all(questionPool.map(entry => {
			return this._summoners.count({
				'applicableQuestions': entry.id
			});
		}))
		.then(applicable_counts => {
			if(applicable_counts.every(count => count > 100))
				return;
	...
	_expandSummonerPool() {
		return this._summoners.findAndModify(
			{ platformId: this._platformId },
			// non-existent dates sort before existing ones
			{ expandDate: 1 },
			{ $currentDate: { expandDate: true } }
		)
		.then(result => {
			assert(result.value);
			console.log(result.value.summonerId);
			return new Promise((resolve, reject) => this._queue.enqueue(() => {
				request({
					url: riotApi.buildUrl({
						apiKey: this._apiKey,
						...
						args: {
							summonerId: result.value.summonerId
						}
					}),
					json: true
				}, (error, res, body) => {
	...
	_purgeSummonerPool() {
		return this._summoners.remove({
			...
			'applicableQuestions': {
				$nin: questionPool.map(entry => entry.id)
			}
		})
