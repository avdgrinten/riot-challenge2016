			...
			this._queue.enqueue(() => {
				request({
					...
					if(res.statusCode == 200) {
						...
						console.error('Retrieving championmastery API failed with status '
								+ res.statusCode);
						resolve(null);
					}
				});
	...
	run() {
		...
		.then(array => {
				...
				this._queue.enqueue(() => {
					request({
						...
						if(res.statusCode == 200) {
							...
							console.error('Retrieving game API failed with status '
									+ res.statusCode);
							resolve(null);
						}
					});
			...
			.then(body => {
				...
				// use reduce here to process the request sequentially
