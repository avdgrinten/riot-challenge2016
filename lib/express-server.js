...
let Server = class {
	...
	setupExpress() {
		...
		app.get('/', (req, res) => {
			this._logic.sampleRandomMasteries().then(sample => {
				let in_lobby_select = !!this._logic.getUser(req.cookies.id);
				let in_summoner_select = true;

				if(in_lobby_select)
					in_summoner_select = false;

				res.set('Content-Type', 'text/html');
				res.send(this._template({
						masteries: sample[0].masteries.map(entry => {
							return {
								championKey: this._logic.getChampion(entry.championId).key
							}
						}),
						lobbyId: this._testLobby.id,
						inLobbySelect: in_lobby_select,
						inSummonerSelect: in_summoner_select
				}));
			});
		});
